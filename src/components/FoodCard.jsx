...
function FoodCard({ food, action }) {
  ...
  return (
    <div className=" bg-[#dd7fc6] text-slate-200 :h-[28vh] w-[32vh] flex flex-col gap-2 justify-between rounded-xl rounded-br-none drop-shadow-md">
      <div className="flex flex-col gap-4 px-4 pt-4">
        <h1 className="text-2xl tracking-wider">{name} </h1>
        <div className="flex flex-col gap-3">
          <p>
            <span className="text-slate-300">Calories:</span> {calories}
          </p>
          <p>
            <span className="text-slate-300">Protein:</span> {protein}g
          </p>
          <p>
            <span className="text-slate-300">Carbs:</span> {carbohydrates}g
          </p>
          <p>
            <span className="text-slate-300">Fat:</span> {fat}g
          </p>
        </div>
      </div>
      <div
        onClick={() => action(_id)}
        className="flex ml-auto cursor-pointer bg-[#ea33bc] hover:bg-red-500 pt-4 pb-2 pl-4 pr-1 rounded-tl-full"
      >
        <svg
